# Coach Team Beta Campaign

Status: draft assets only. Nothing in this plan has been sent or published outside the website branch.

## Offer

Give a high school track or cross-country coach a free OneCommit team kickoff:

- athlete-owned beta accounts
- a one-page profile and school-list checklist
- one 20-minute coach kickoff
- a weekly athlete check-in rhythm
- direct support for access problems

This is not a coach dashboard, recruiting agency, roster-monitoring product, or promise of coach replies.

## Audience

Start with head and assistant coaches who:

- coach track and field or cross-country in the United States
- have juniors or seniors actively considering college athletics
- already use email, spreadsheets, or shared documents for recruiting check-ins
- are willing to test with 5-15 athletes before a full-team rollout

## Message

Primary line:

> Give every athlete a recruiting plan without becoming their agent.

Supporting proof:

- athletes build and own their profiles
- current beta matching is D3-focused and track-specific; confidence should be disclosed when data is thin
- outreach sends from the athlete's Gmail or Outlook account
- communication history stays organized by school
- beta access is free while timing and refinement recommendations are validated

Do not market automatic timing intelligence, a coach portal, unlimited sends, equal all-division coverage, or guaranteed recruiting outcomes.

## Four-Week Test

### Week 1: Warm discovery

- Build a reviewed list of 25 coaches from existing relationships and public school staff pages.
- Ask 5 coaches for a 15-minute workflow interview before asking them to promote the app.
- Record current process, roster size, biggest recruiting bottleneck, and which athletes need help now.

### Week 2: Small-team beta

- Invite 3 coaches to start with 5-15 athletes each.
- Give each coach the same kickoff outline and athlete checklist.
- Track coach-page visits, `coach_interest_click`, replies, athletes who receive access, profiles completed, and first school saved.

### Week 3: Activation review

- Ask coaches which athletes stalled and why.
- Measure profile completion, first matched-school view, first saved school, and first reviewed outreach.
- Do not count raw email volume as success.

### Week 4: Decision

- Continue only if at least 2 coaches run a second weekly check-in and athletes reach meaningful setup milestones.
- Revise the page and kickoff based on observed friction.
- Keep beta free; do not create public paid coach pricing from this test.

## Email Draft A: Workflow Interview

Subject: A simpler recruiting check-in for your track athletes

Coach [Last Name],

I am building OneCommit, a track-specific recruiting workspace that helps athletes organize matched schools, prepare personal outreach, and track replies from their own inbox.

The goal is not to give coaches another portal. It is to give each athlete enough structure that a weekly recruiting check-in can focus on decisions instead of rebuilding the same spreadsheet.

Could I ask you 4 questions about how your athletes handle recruiting now? I am looking for honest workflow feedback, not asking you to buy anything.

Hugh

## Email Draft B: Team Beta

Subject: Free OneCommit beta for a small athlete group

Coach [Last Name],

Based on what coaches told us, we are opening a small free beta for track and cross-country teams. Athletes own their accounts and messages; OneCommit gives them a school-list and outreach workspace you can review in a short weekly check-in.

We can start with 5-15 athletes and send a one-page kickoff outline. There is no coach dashboard, no charge during beta, and no promise of recruiting outcomes.

Would a small group on your team be useful this month?

Hugh

## Measurement

Use this funnel:

1. coach page view
2. `coach_interest_click`
3. coach reply
4. beta access delivered
5. athlete profile completed
6. first school saved
7. first reviewed outreach
8. second coach-led weekly check-in

The campaign succeeds when coaches repeat the workflow and athletes complete useful recruiting actions, not when the site generates impressions or email volume.
