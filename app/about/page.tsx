    ...
    <RouteShell>
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        <section
          ...
          className={`${hero} oc-wash relative overflow-x-clip`}
        >
        ...
        <section aria-labelledby="purpose-heading" className={section}>
          <div
            className={`${container} grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:gap-20`}
          >
            <Reveal>
            ...
            <Reveal delay={0.1} className={`space-y-5 ${prose}`}>
              <p>
                Athletes often have their marks, academics, school research, and
                coach conversations spread across disconnected tools. OneCommit
                is intended to make that work easier to inspect and act on
                without taking control away from the athlete.
              </p>
              <p>
                The product is decision support. It can help organize evidence
                and next steps, but it cannot know a coach&apos;s final
                evaluation or guarantee an outcome.
              </p>
            </Reveal>
          </div>
        </section>
        ...
        <section
          ...
          className="oc-band bg-shell px-4 py-24 text-white sm:px-6 lg:py-32"
        >
          <div className={container}>
            ...
            <RevealGroup className="mt-14 grid gap-10 md:grid-cols-3 md:gap-8">
              {principles.map((principle, index) => (
                ...
                >
                  <p className="text-[13px] font-semibold tracking-[0.08em] text-mint">
                    0{index + 1}
                  </p>
            ...
            <Reveal delay={0.1}>
              <ul className="divide-y divide-line border-y border-line">
                {betaFacts.map((fact) => (
                  <li
                    key={fact}
                    className="py-5 text-[17px] leading-[1.6] text-ink sm:text-[18px]"
                  >
                    {fact}
                  </li>
                ))}
        ...
        <section aria-labelledby="accountability-heading" className={section}>
          {/* Nested, not `${container} max-w-[46rem]` — two max-width utilities in one
              class string collide and the wider wins on emission order. The rail is the
              outer box; the measure is the inner one. A bare max-w-3xl here put this
              band's eyebrow 216px inboard of every other heading on the page. */}
          <div className={container}>
            <Reveal className="w-full max-w-[46rem]">
              <p className={eyebrow}>Accountability</p>
              <h2 id="accountability-heading" className={`mt-4 ${h2}`}>
                Ask us directly.
              </h2>
              <p className={`mt-4 ${lede}`}>
                Questions about access, product boundaries, privacy, or account
                data should have a clear path to a real answer. Email the team
                or review the policies that govern the service.
              </p>
