...
// The homepage runs its own nav, so the same target-size rule has to be asserted
// against it or the primary conversion control sits outside the rule.
// site-nav writes its classes as a template literal, so this cannot require the
// double-quoted className="..." form the public-header assertions above use.
assert.match(
  homeNavSource,
  /aria-label="OneCommit home"[\s\S]{0,320}?\bmin-h-11\b/,
  "home nav home link must retain a 44px minimum target",
);
assert.match(
  homeNavSource,
...
// These three controls build from components/home/tokens.ts instead of spelling the
// classes inline, so the rule follows the recipe: each control must use the shared
// pill, and the shared pill must carry the 44px floor.
assert.match(
  tokensSource,
  /const pillBase[\s\S]{0,400}?\bmin-h-11\b/,
  "the shared pill recipe must keep a 44px minimum target",
);
assert.match(
  downloadSource,
  /eventSource="download_page_fallback_email"[\s\S]{0,420}?\bpillPrimary\b/,
  "download fallback action must use the shared primary pill",
);
assert.match(
  downloadSource,
  /analyticsSource="download_page_primary"[\s\S]{0,420}?\bpillPrimary\b/,
  "download primary action must use the shared primary pill",
);
assert.match(
  downloadSource,
  ...
  for (const routeCase of routeCases) {
    ...
    if (route === "/download") {
      for (const anchor of mainAnchors(html)) {
        assertMinimumTarget(anchor, "min-h-11", "/download main actions must each be at least 44px tall");
      }
